...
class ControllerExtensionPaymentPaydo extends Controller {
    ...
    public function pay() {
        ...
        // Считаем amount так же, как в OC4: 2 знака после запятой
        ...
        // Подпись как в OC4 — по массиву order
        $request['signature'] = $this->generate_order_signature($request['order']);

        // Переводим заказ в статус ожидания
        ...
        if ($invoiceId === '') {
            ...
            // Можно залогировать, чтобы видеть реальный ID
            $this->log->write('Paydo invoiceId: ' . $invoiceId);

            $redirectUrl = "https://checkout.paydo.com/{$this->language->get('code')}/payment/invoice-preprocessing/{$invoiceId}";
            // Возвращаем просто строку-URL (jQuery её съест)
    ...
    public function callback() {
        ...
        // Новый формат (как в OC4): invoice + transaction
        if ($callback && isset($callback['invoice'])) {
            ...
            // Легаси формат (orderId / amount / currency / status / signature) – на всякий случай оставим
    ...
    private function makeRequest($request = array()) {
        ...
        if (!$this->curl) {
            ...
            curl_setopt($this->curl, CURLOPT_HEADER, false); // только тело, без заголовков
        }
